C uel_mos.f --- 3D NMOS device simulation UEL (TCAD in Abaqus).
C 드리프트-확산을 실제 TCAD(Silvaco/Sentaurus)처럼 box method + 준페르미
C 변수로 푼다: 절점 자유도 psi/VT(1), phi_n/VT(2), phi_p/VT(3),
C   n = ni exp(psi - phi_n),  p = ni exp(phi_p - psi)   (유계 전압 미지수)
C 축정렬 육면체 12 모서리마다 Scharfetter & Gummel (1969) 플럭스.
C 검증: 긴채널 NMOS I_D 를 Pao & Sah (1966) / Brews (1978)와 대조.
C 단위: 길이 cm, 퍼텐셜 V_T, 농도 n_i. 전류 복원 I = q*ni*RF(dof 2).
C PROPS: (1) 0=산화막/1=실리콘, (2) net doping / ni (부호 포함)
...
C       SG 플럭스 (a -> b), t = psi_b - psi_a
        T = U(KB) - U(KA)
        CALL EX17B(T,  BT,  BPT)
        CALL EX17B(-T, BMT, BPMT)
        ENA = EN(IA)
        ENB = EN(IB)
        PA  = P(IA)
        PB  = P(IB)
